...
template <class Handler, typename = typename std::enable_if<is_handler<Handler>::value>::type> 
class basic_parser {
public:
    typedef enum {
        BEFORE_FIELD,
...
private:
    ...
    inline void on_field(std::string_view field) {
        entry_.first.append(field.data(), static_cast<std::string::size_type>(field.size()));
    }
